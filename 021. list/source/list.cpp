...
int main()
{
	...
	for (auto iter = item_list.begin(); iter != item_list.end(); )
	{
		...
		if (*iter == "곡괭이")
		{
			...
			// 다음 주소를 반환하기 때문에
			// iter로 다음 주소를 받아야 함!
			// erase()가 iter를 삭제하므로 ++iter로는
			// 다음 값을 알아낼 수 없음!
			// 하지만 vector에서는 빈 자리가 채워지므로
			// 다음 값을 알아낼 수 있음.
			// 하지만 vector에서 ++iter를 하게 되면 다다음 값으로 접근함
			// 결론을 내리자면 iter = erase(iter)로 이용하고
			// ++iter는 따로 이용해야 함
			// 
			// erase(iter++)는 주소를 하나 넘어가므로 추천하지 않음
			// 아까도 설명했지만 erase()를 사용할 때는 ++iter를 따로 쓰는 게 좋음!
